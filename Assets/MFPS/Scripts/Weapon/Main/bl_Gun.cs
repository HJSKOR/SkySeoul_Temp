...
public class bl_Gun : bl_GunBase
{
    ...
    void SingleFire()
    {
        if (PlayerReferences.firstPersonController.State is PlayerState.Running or PlayerState.Crouching) return;
    ...
    public void BuildBulletData()
    {
        ...
        BulletSettings.MFPSActor = new MFPSPlayer();//bl_GameManager.Instance.LocalActor;
    ...
    {
        // TODO
        CanFire = true;//!bl_GameManager.Instance.GameFinish;
        CanAiming = true;
        if (inReloadMode)
        {
            if (WeaponType == GunType.Grenade)
