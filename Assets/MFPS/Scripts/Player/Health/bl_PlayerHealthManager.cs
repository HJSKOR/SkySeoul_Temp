...
public class bl_PlayerHealthManager : bl_PlayerHealthManagerBase
{
    ...
    public void GetDamage(DamageData e)
    {
        ...
        SyncDamage( e.Damage, e.From, e.Cause, e.Direction, e.isHeadShot, e.GunID, bl_PhotonNetwork.LocalPlayer);
    ...
    public override void DoFallDamage(int damage)
    {
        ...
        SyncDamage(damage, bl_PhotonNetwork.NickName, DamageCause.FallDamage, downpos, false, 103, bl_PhotonNetwork.LocalPlayer);
    }
	...
	void Die(string killer, bool isHeadshot, DamageCause cause, int gunID, Vector3 hitPos, Player sender)
    {
        ...
        else
        {
            ...
            {
                ForcePosition = hitPos,
                Velocity = Vector3.one,//m_CharacterController.velocity,
                IsFromExplosion = isExplosion,
                AutoDestroy = true,
                RightHandChild = ngr
            });
